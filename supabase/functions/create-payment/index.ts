...
serve(async (req) => {
  try {
    ...
    if (!amount || !role) {
      return new Response(JSON.stringify({ error: "amount and role are required" }), { status: 400 })
    }

    // Convert AED to fils (base unit)
    ...
    if (!res.ok) {
      return new Response(JSON.stringify({ error: data.message || "Ziina API error" }), { status: res.status })
    }
